    ...
    async def assess(
        ...
        *,
        ...
        prompt = (
            ...
            "SPECIAL INVALID-INVITE WORKFLOW: If the recipient says an invite is invalid, do not "
            "hand off immediately and do not claim the invite or application has been validated. "
            "Unless the transcript already contains both the applied-for role and the Discord "
            "invitation itself, set `clear` false and ask for them in exactly this form format: "
            "APPLIED FOR ROLE and DISCORD INVITE. Do not ask for Roblox username, Discord username, "
            "or Discord ID. "
            "First determine whether the request concerns the Academy; non-Academy requests such as "
            "transfer training should be handed to Training & Recruitment after the required context "
            "is collected. A replacement invite may only be considered after the Academy system "
            "confirms a successful application and the submitted invite is confirmed invalid and "
            "unused. If the invite was used, or the successful application cannot be confirmed, do "
            "not provide an invite and hand off to Training & Recruitment. "
            "`resolved` may "
